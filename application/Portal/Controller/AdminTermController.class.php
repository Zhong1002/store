...
class AdminTermController extends AdminbaseController {
	...
	function index(){
		...
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		foreach ($result as $r) {
			$r['str_manage'] = '<a href="' . U("AdminTerm/add", array("parent" => $r['type_id'])) . '">'.L('ADD_SUB_CATEGORY').'</a> | <a href="' . U("AdminTerm/edit", array("id" => $r['type_id'])) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . U("AdminTerm/delete", array("id" => $r['type_id'])) . '">'.L('DELETE').'</a> ';
			$url=U('Book/Category/category',array('tid'=>$r['type_id']));
			$r['url'] = $url;
	...
	function add(){
	 	...
	 	foreach ($terms as $r) {
	 		$r['id']=$r['type_id'];
	 		$r['parentid']=$r['parent'];
	 		$r['selected']= (!empty($parentid) && $r['type_id']==$parentid)? "selected":"";
	 		$new_terms[] = $r;
	 	}
	 	$tree->init($new_terms);
	...
	function edit(){
		$id = intval(I("get.id"));
		$data=$this->terms_model->where(array("type_id" => $id))->find();
		$tree = new \Tree();
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		$terms = $this->terms_model->where(array("type_id" => array("NEQ",$id)))->select();
		
		$new_terms=array();
		foreach ($terms as $r) {
			$r['id']=$r['type_id'];
			$r['parentid']=$r['parent'];
			$r['selected']=$data['parent']==$r['type_id']?"selected":"";
			$new_terms[] = $r;
		}
