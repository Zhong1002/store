 ...
 * 会员注册
 */
namespace StoreUser\Controller;
...
//             }
    	    ...
    	    if($result){
    	       ...
    	       if($result!==false){
    	           $this->success("密码重置成功，请登录！",U("StoreUser/login/index"));
    	       }else{
    	           $this->error("密码重置失败！");
    	       }
    	    }else{
    ...
    private function _do_mobile_login(){
        ...
        if(!empty($result)){
            if(sp_compare_password($password, $result['password'])){
                ...
                $redirect=empty($session_login_http_referer)?U('Book/Index/index'):$session_login_http_referer;
