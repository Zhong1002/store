<?php
namespace Admin\Controller;

use Common\Controller\AdminbaseController;

class SlideController extends AdminbaseController{
	
	protected $slide_model;
	protected $slidecat_model;
	
	public function _initialize() {
		parent::_initialize();
		$this->slide_model = M("AppAd");
// 		$this->slidecat_model = D("Common/SlideCat");
	}
	
	public function index(){
		/*$cates=array(
			array("cid"=>"0","cat_name"=>"默认分类"),
		);
		$categorys=$this->slidecat_model->field("cid,cat_name")->where("cat_status!=0")->select();
		if($categorys){
			$categorys=array_merge($cates,$categorys);
		}else{
			$categorys=$cates;
		}
		$this->assign("categorys",$categorys);
		$where=array();
		$cid = I('post.cid',0,'intval');
		if(!empty($cid)){
			$where=array('slide_cid'=>$cid);
		}
		$this->assign("slide_cid",$cid);*/
		$slides=$this->slide_model->where()->order("listorder ASC")->select();
		$this->assign('slides',$slides);
		$this->display();
	}
	
	public function add(){
// 		$categorys=$this->slidecat_model->field("cid,cat_name")->where("cat_status!=0")->select();
// 		$this->assign("categorys",$categorys);
		$this->display();
	}
	
	public function add_post(){
		if(IS_POST){
			$data = $this->slide_model->create();
			if ($data!==false) {
				$data['create_time'] = time();
				if ($this->slide_model->add($data)!==false) {
					$this->success("添加成功！", U("slide/index"));
				} else {
					$this->error("添加失败！");
				}
			} else {
				$this->error($this->slide_model->getError());
			}
		}
	}
	
	public function edit(){
// 		$categorys=$this->slidecat_model->field("cid,cat_name")->where("cat_status!=0")->select();
		$id = I("get.id",0,'intval');
		$slide=$this->slide_model->where(array('id'=>$id))->find();
		$this->assign($slide);
// 		$this->assign("categorys",$categorys);
		$this->display();
	}
	
	public function edit_post(){
		if(IS_POST){
			$data = $this->slide_model->create();
			if ($data!==false) {				//如果创建了模型的话在创建数据的时候需要验证
				$data['create_time'] = time();
				if ($this->slide_model->save()!==false) {
					$this->success("保存成功！", U("slide/index"));
				} else {
					$this->error("保存失败！");
				}
			} else {
				$this->error($this->slide_model->getError());
			}
				
		}
	}
	
	public function delete(){
		if(isset($_POST['ids'])){
			$ids = implode(",", $_POST['ids']);
			$data['slide_status']=0;
			if ($this->slide_model->where("id in ($ids)")->delete()!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		}else{
			$id = I("get.id",0,'intval');
			if ($this->slide_model->delete($id)!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		}
		
	}
	
	public function toggle(){
		if(isset($_POST['ids']) && $_GET["display"]){
			$ids = I('post.ids/a');
			if ($this->slide_model->where(array('id'=>array('in',$ids)))->save(array('slide_status'=>1))!==false) {
				$this->success("显示成功！");
			} else {
				$this->error("显示失败！");
			}
		}
		if(isset($_POST['ids']) && $_GET["hide"]){
			$ids = I('post.ids/a');
			if ($this->slide_model->where(array('id'=>array('in',$ids)))->save(array('slide_status'=>0))!==false) {
				$this->success("隐藏成功！");
			} else {
				$this->error("隐藏失败！");
			}
		}
	}
	
	//隐藏
	public function ban(){
    	$id = I('get.id',0,'intval');
    	if ($id) {
    		$rst = $this->slide_model->where(array('id'=>$id))->save(array('slide_status'=>0));
    		if ($rst) {
    			$this->success("幻灯片隐藏成功！");
    		} else {
    			$this->error('幻灯片隐藏失败！');
    		}
    	} else {
    		$this->error('数据传入失败！');
    	}
    }
    
    //显示
    public function cancelban(){
    	$id = I('get.id',0,'intval');
    	if ($id) {
    		$result = $this->slide_model->where(array('id'=>$id))->save(array('slide_status'=>1));
    		if ($result) {
    			$this->success("幻灯片启用成功！");
    		} else {
    			$this->error('幻灯片启用失败！');
    		}
    	} else {
    		$this->error('数据传入失败！');
    	}
    }
    
	//排序
	public function listorders() {
		$status = parent::_listorders($this->slide_model);
		if ($status) {
			$this->success("排序更新成功！");
		} else {
			$this->error("排序更新失败！");
		}
	}
	
}